...
class BinaryConv2d(nn.Conv2d):
    ...
    def forward(self, input):
        # out = F.conv2d(input, self.binarize(self.weight), self.bias, self.stride, self.padding, self.dilation, self.groups)
...
class PGBinaryConv2d(nn.Conv2d):
    ...
    def forward(self, input):
        ''' MSB convolution '''
        # out_msb = F.conv2d(self.binarize(input), self.binarize(self.weight), self.bias, self.stride, self.padding, self.dilation, self.groups) * 2.0 / 3.0
        out_msb = F.conv2d(self.binarize(input), self.weight, self.bias, self.stride, self.padding, self.dilation, self.groups) * 2.0 / 3.0
        out_msb = out_msb * self.alpha
        ''' Calculate the mask '''
        ...
        ''' full convolution '''
        # out_full = F.conv2d(input, self.binarize(self.weight), self.bias, self.stride, self.padding, self.dilation, self.groups)
        out_full = F.conv2d(input, self.weight, self.bias, self.stride, self.padding, self.dilation, self.groups)
        out_full = out_full * self.beta
        ''' combine outputs '''
...
class InputEncoder(nn.Module):
    ...
    def __init__(self, input_size, resolution):
        ...
        placeholder = torch.ones(self.n, self.c, self.b, self.h, self.w, dtype=torch.float32).cuda() \
                                 if torch.cuda.is_available() else torch.ones(self.n, self.c, self.b, self.h, self.w, dtype=torch.float32)
        placeholder *= torch.arange(self.b).view(1, 1, -1, 1, 1).cuda() if torch.cuda.is_available() else torch.arange(self.b).view(1, 1, -1, 1, 1)
        self.register_buffer('placeholder', placeholder)

    def forward(self, x):
        x = (x * 5.0).view(-1, self.c, 1, self.h, self.w)
        if x.size(0) != self.n:
            self.n = x.size(0)
            self.placeholder = torch.ones(self.n, self.c, self.b, self.h, self.w,
                                          dtype=torch.float32).cuda() if torch.cuda.is_available() else torch.ones(
                self.n, self.c, self.b, self.h, self.w, dtype=torch.float32)
            self.placeholder *= torch.arange(self.b).view(1, 1, -1, 1,
                                                          1).cuda() if torch.cuda.is_available() else torch.arange(
                self.b).view(1, 1, -1, 1, 1)
